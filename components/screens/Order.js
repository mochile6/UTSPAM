...
const SearchResult = ({data}) => {
  ...
  var listJadwal = () => {
    var newJadwal = Jadwal.filter(x => {
      let checkPelabuhanK = jadwal.pelabuhan_awal.filter(x => {
        return (
          x.pelabuhan_awal.toLowerCase() == data.keberangkatan.toLowerCase()
        );
      });

      let checkPelabuhanT = Jadwal.pelabuhan_tujuan.filter(x => {
        return x.pelabuhan_tujuan.toLowerCase() == data.tujuan.toLowerCase();
      });

      try {
        return (
          x.bandara_kode_keberangkatan.toLowerCase() ==
            checkPelabuhanK[0].bandara_kode.toLowerCase() &&
          x.bandara_kode_tujuan.toLowerCase() ==
            checkPelabuhanT[0].bandara_kode.toLowerCase() &&
            ...
            <View style={resultStyle.box} key={i}>
              <View style={resultStyle.upperText}>
                <Text style={resultStyle.text}>
                  {KapitalFirstsletter(data.keberangkatan)}
                </Text>
                <Text style={resultStyle.text}> - </Text>
                <Text style={resultStyle.text}>
                  {KapitalFirstsletter(data.tujuan)}
                </Text>
              </View>
      ...
      return (
        <View style={resultStyle.boxNotAvail}>
          <Text style={resultStyle.textNotAvail}>
            Maaf, jadwal penerbangan tidak tersedia{' '}
          </Text>
